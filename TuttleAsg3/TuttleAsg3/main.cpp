...
//  TuttleAsg3
//
//  Created by Anastasia Tuttle  on 2/8/24.
//
...
void writeToFile(int device_id)
{
    string filename = "device_" + to_string(device_id) + "output.txt";
    ofstream file(filename, ios::app);
    
    lock_guard<mutex> lock(queueMutex);
    while(!inputQueue.empty())
    {
        file << inputQueue.back() <<endl;
        inputQueue.pop_back();
    }
}
...
int main(int argc, char *argv[])
{
    ...
    //Create and launch 10 reader threads
    thread readerThreads[10];
    for (int i = 0; i < 10; ++i)
    {
        readerThreads[i] = thread(writeToFile, i);
    }
    ...
    while (chrono::duration_cast<chrono::minutes>(chrono::system_clock::now() - startTime).count() < 5)
    {
        {
            lock_guard<mutex> lock(queueMutex);
            cout<<"Queue length: "<<inputQueue.size()<<endl;
        }
        this_thread::sleep_for(chrono::milliseconds(1000));
    }
    //Turn off the writer thread
    {
        lock_guard<mutex> lock(queueMutex);
        inputThread.join();
    }
    //Give reader threads a few moments to empty the queue
